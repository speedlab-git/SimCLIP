...
def main(args):
    ...
    # Original model_orig is the model that we are going to attack
    ...
    if args.dataset == 'imagenet':
        ...
        dataset = SamData('/data/naman_deep_singh/datasets/newSAM', transform=preprocessor_without_normalize) #transform: preprocessor 
    ...
    #Original to main device
    
## This portion for getting the text embeddings
#=========================================================================
    with torch.no_grad():
...
def train_one_epoch(
    ...
    print("embedding",embedding_text_labels_norm)
    model_orig.eval()
    model.train()
    ...
    for i, (data, targets) in enumerate(dataloader):
        ...
        # print(len(data_adv))
        embedding_clean = model(data, output_normalize=args.output_normalize) #from data clean embedding 
        ...
        # loss_total= -F.cosine_similarity(embedding_adv, embedding_orig.detach(), dim=-1).mean()
